...
Route::group(['prefix' => 'dashboard', 'middleware' => 'auth'], function () {
    ...
    Route::post('role/delete/permission/{role}', [App\View\Components\Dashboard\role\permission\Manage::class, 'delete'])->name('role.delete.permission');
     //Route::match(['delete', 'post'], 'delete/permission/{role}', [RoleManage::class, 'delete'])->name('role.delete.permission');
...
Route::get('/polimor', function () {
    ...
    */
    /*
     ...
     $product->image()->create(['url' => 'profiles/profile1.jpg']);
     */
    
     $product = Product::find(2);
     $product->image()->create(['url' => 'profiles/producto2.jpg']);
    ...
    */
    ...
    dd($owner);
    echo "La imagen pertenece a: " . get_class($owner) . " con ID: " . $owner->id;
    */
});

Route::middleware('auth')->group(function () {
    Route::get('/roles', function () {
        $rol = Auth::user()->hasRole( 'Admin') ? "Es admin" : "No es admin";
        echo $rol;
        $rol = Auth::user()->can('editor.post.update') ? "<br>Es editor puede actualiza" : "<br>No es editor no puede actualizar";
        echo $rol;
    });
});
